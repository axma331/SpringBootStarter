...
<details>
<summary><span style="color:red">Важная информация при подключении к другому проекту!</span></summary>

Если вы подключаете `HTTP Logging Spring Boot Starter` в качестве зависимости в другой проект, обратите внимание на следующее:

- **Удаление плагина `spring-boot-maven-plugin`:**
  Если в POM файле вашего стартер-пакета присутствует плагин:
  ```xml
  <plugin>
  ...
  ```
- Этот плагин должен быть удален из POM файла вашего стартер-пакета при подключении его в качестве зависимости в другом проекте. Наличие этого плагина в подключаемом проекте может вызвать конфликты, особенно если основной проект также использует этот плагин.
- Важно, чтобы сборка и управление Spring Boot приложением оставалось под контролем основного проекта, а не стартер-пакета.
</details>
